...
public class PokerTableController implements Initializable {
	...
	public void btnDrawCard_Click(ActionEvent event) {
		...
		for (int iPosition = 1; iPosition < 3; iPosition++) {
			...
			RotateTransition rotT2 = CreateRotateTransition2(img);
			...
			ParallelTransition patFadeNSpin = new ParallelTransition();
			patFadeNSpin.getChildren().addAll(patTFadeInFadeOut, rotT2);

			SequentialTransition seqDeal = new SequentialTransition();

			seqDeal.getChildren().addAll(patTMoveRot, patFadeNSpin);
	...
	private PathTransition CreatePathTransition(Point2D fromPoint, Point2D toPoint, ImageView img) {
		...
		path.getElements().add(new CubicCurveTo(toPoint.getX()+33 * 2, toPoint.getY() * 2, toPoint.getX()+33 / 3,
			toPoint.getY() / 3, toPoint.getX()+33, toPoint.getY()+5));
	...
	private RotateTransition CreateRotateTransition2(ImageView img) {

		RotateTransition rotateTransition = new RotateTransition(Duration.millis(iAnimationLength), img);
		rotateTransition.setByAngle(285F);
		rotateTransition.setCycleCount(2);
		rotateTransition.setAutoReverse(false);
